...
OUT_DIR = "analysis/outputs/wavelet_2021"
...
TRIM_H = 24     # remove 24h at both ends to reduce edge effects

# coherence smoothing (simple + stable)
SMOOTH_TIME_H = 24      # ~1 day smoothing in time
SMOOTH_SCALE_BINS = 3   # smoothing across adjacent scales
...
def cwt(x: np.ndarray, periods_h: np.ndarray) -> np.ndarray:
    """Return complex CWT coefficients W (n_scales, n_time) using Morlet."""
    x0 = x.copy()
    x0[np.isnan(x0)] = 0.0
...
def smooth_time_same(mat: np.ndarray, win: int) -> np.ndarray:
    ...
    if win <= 1:
        return mat
    if win % 2 == 0:
        win += 1  # force odd for symmetric smoothing

    pad = win // 2
    kernel = np.ones(win, dtype=float) / win

    # reflect-pad time axis
    mp = np.pad(mat, ((0, 0), (pad, pad)), mode="reflect")

    out = np.empty_like(mat, dtype=float if np.isrealobj(mat) else complex)
    for i in range(mat.shape[0]):
        out[i, :] = np.convolve(mp[i, :], kernel, mode="valid")  # valid returns same length as original
    return out
...
def coherence_from_cwts(Wx: np.ndarray, Wy: np.ndarray, valid_mask_2d: np.ndarray) -> np.ndarray:
    ...
    # Set invalid to 0 (weights will prevent influence)
    ...
    # Smooth numerator and denominators + weights
    ...
    # Mask invalid back to NaN for reporting/averaging
...
def main():
    ...
    for fp in files:
        ...
        # Build valid mask across scales/time (must match Wx time length)
        valid_t = ~(mask_in | mask_out)
        valid_t = valid_t[:Wx.shape[1]]  # safety
        ...
        band_rows.append({"pair": base, **bands})
        ...
        plt.title(f"Wavelet coherence R² | {base}")
        plt.xlabel("time (trimmed hours)")
        plt.ylabel("period")
        plt.colorbar()
        plt.tight_layout()
        plt.savefig(os.path.join(FIG_DIR, f"{base}_coherence_matrix.png"), dpi=200)
        plt.close()

        print(f"[OK] coherence for {base} | mean_R2={mean_R2:.3f} | valid_frac={frac_valid:.3f}")
