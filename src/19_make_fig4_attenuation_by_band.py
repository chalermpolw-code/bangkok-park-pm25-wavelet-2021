...
ATT_PATH = PROJECT_ROOT / "analysis" / "outputs" / "wavelet_2021" / "attenuation_bandmeans.csv"
OUT_DIR = PROJECT_ROOT / "manuscript" / "figures"
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_PDF = OUT_DIR / "Fig4_attenuation_by_band_2021.pdf"
OUT_PNG = OUT_DIR / "Fig4_attenuation_by_band_2021.png"
...
# Professional Q1 Color Palette
COLOR_BOX_FACE = "#f1f3f5"
COLOR_BOX_EDGE = "#495057"
COLOR_MEDIAN = "#d95f02"    # Bold contrasting median line
COLOR_POINTS = "#2b83ba"    # Professional teal/blue for data points
COLOR_ZERO = "#212529"
...
def melt_long(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df[cols].copy()
    out = out.melt(id_vars=["pair"], var_name="band", value_name="attenuation_db")
...
def main() -> None:
    ...
    att_long = melt_long(att)

    fig, ax = plt.subplots(figsize=FIGSIZE)

    data = [
        att_long.loc[att_long["band"] == b, "attenuation_db"].dropna().values
        for b in BANDS
    ]
    ...
    ax.boxplot(
        ...
        medianprops=dict(color=COLOR_MEDIAN, linewidth=2.5), # Highlight median
        zorder=1
    )

    # Deterministic small jitter for individual parks
    for i, b in enumerate(BANDS, start=1):
        sub = att_long.loc[att_long["band"] == b, ["pair", "attenuation_db"]].dropna()
        n = len(sub)
        if n == 0:
            continue
        xs = [i + (j - (n - 1) / 2) * 0.04 for j in range(n)] # Slightly widened spread

        # Q1-styled Scatter Points
        ax.plot(
            xs,
            sub["attenuation_db"].values,
            marker="o",
    ...
    ax.axhline(0.0, color=COLOR_ZERO, linewidth=1.2, linestyle="--", zorder=0)

    ax.set_ylabel(r"Band-mean attenuation (dB), $10\log_{10}(P_{\mathrm{in}}/P_{\mathrm{out}})$", fontsize=12, labelpad=10)
